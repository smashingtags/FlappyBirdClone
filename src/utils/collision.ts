...
export const createCollisionBox = (x: number, y: number, width: number, height: number): CollisionBox => ({
  x,
  y,
  width,
  height,
...
export const checkAABBCollision = (box1: CollisionBox, box2: CollisionBox): boolean => {
  return (
...
export const getPipeCollisionBoxes = (pipe: PipeState): CollisionBox[] => {
  ...
  const bottomPipe = createCollisionBox(
    pipe.x,
    SCREEN_DIMENSIONS.HEIGHT - pipe.bottomHeight - SCREEN_DIMENSIONS.GROUND_HEIGHT,
    PHYSICS_CONFIG.PIPE_WIDTH,
    pipe.bottomHeight
  );
...
export const checkBirdPipeCollision = (bird: BirdState, pipe: PipeState): boolean => {
...
export const checkBirdGroundCollision = (bird: BirdState): boolean => {
  return bird.y + PHYSICS_CONFIG.BIRD_SIZE.height > SCREEN_DIMENSIONS.HEIGHT - SCREEN_DIMENSIONS.GROUND_HEIGHT;
};
...
export const checkBirdPassedPipe = (bird: BirdState, pipe: PipeState): boolean => {
